...
namespace stdext
	{
		namespace impl
			{
				...
				template <typename T, typename Enable = void> class mmaparray_base: public array_term <T>
					{
						protected:
						...
						private:
							macstl::mmapping <T> mmap_;
					};
	
				template <typename T> class mmaparray_base <T, typename enable_if <exists <typename array_term <T>::chunk_type>::value>::type>:
					public array_term <T>
					{
						public:
							...
							mmaparray_base (const macstl::channel& chan, macstl::mmap_mode::mode mode, std::size_t n, std::size_t pos):
								mmap_ (chan, mode, (n + chunk_type::length - 1) / chunk_type::length, pos)
								{
						...
						private:
							macstl::mmapping <chunk_type> mmap_;
		...
		template <typename T> class mmaparray: public impl::mmaparray_base <T>
			{
				public:
					...
					mmaparray (const macstl::channel& chan, macstl::mmap_mode::mode mode = macstl::mmap_mode::rdonly, std::size_t n = 0, std::size_t pos = 0):
						impl::mmaparray_base <T> (chan, mode, n, pos)
						{
						}
					...
					template <typename Expr> mmaparray& operator= (const impl::term <T, Expr>& other)
						{
